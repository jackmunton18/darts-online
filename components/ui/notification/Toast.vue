<template>
    <div class="fixed bottom-6 right-6 z-50 flex flex-col gap-4 max-w-md">
        <transition-group name="notification">
            <div 
                v-for="notification in messages" 
                :key="notification.id" 
                :class="[
                    'rounded-md shadow-lg p-4 flex gap-2 items-start transform transition-all duration-300',
                    getNotificationColorClass(notification.type)
                ]"
            >
                <div :class="['flex-shrink-0', getIconColorClass(notification.type)]">
                    <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        <path v-if="notification.type === 'success'" fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                        <path v-else-if="notification.type === 'error'" fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
                        <path v-else-if="notification.type === 'warning'" fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                        <path v-else fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
                    </svg>
                </div>
                <div class="flex-1">
                    <p class="text-sm font-medium">{{ notification.message }}</p>
                </div>
                <div class="flex-shrink-0">
                    <button 
                        @click="clearMessage(notification.id!)" 
                        class="text-gray-400 hover:text-gray-500"
                    >
                        <span class="sr-only">Close</span>
                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
                    </button>
                </div>
            </div>
        </transition-group>
    </div>
</template>

<script setup lang="ts">
import { useNotificationStore } from '~/stores/notification'
import { onMounted, onBeforeUnmount, watch } from 'vue'

const toastStore = useNotificationStore()
const { messages, clearMessage } = toastStore

// Map to store timeout IDs for each notification
const timeoutIds = new Map()

// Function to set timeout for a notification
const setNotificationTimeout = (notification: any) => {
    if (notification && notification.id && notification.timer) {
        // Clear any existing timeout for this notification
        if (timeoutIds.has(notification.id)) {
            clearTimeout(timeoutIds.get(notification.id))
        }
        
        // Set new timeout
        const timeoutId = setTimeout(() => {
            clearMessage(notification.id)
        }, notification.timer)
        
        // Store timeout ID
        timeoutIds.set(notification.id, timeoutId)
    }
}

// Watch for new notifications and set timeouts
watch(() => messages.length, (newLength, oldLength) => {
    if (newLength > oldLength) {
        // New notification has been added
        const newNotification = messages[messages.length - 1]
        setNotificationTimeout(newNotification)
    }
})

// Set up timeouts for any existing messages
onMounted(() => {
    messages.forEach(notification => {
        setNotificationTimeout(notification)
    })
})

// Clean up timeouts when component is unmounted
onBeforeUnmount(() => {
    timeoutIds.forEach(id => clearTimeout(id))
    timeoutIds.clear()
})

const getNotificationColorClass = (type: string) => {
    switch (type) {
        case 'success':
            return 'bg-green-50 text-green-800'
        case 'error':
            return 'bg-red-50 text-red-800'
        case 'warning':
            return 'bg-yellow-50 text-yellow-800'
        case 'info':
            return 'bg-blue-50 text-blue-800'
        default:
            return 'bg-gray-50 text-gray-800'
    }
}

const getIconColorClass = (type: string) => {
    switch (type) {
        case 'success':
            return 'text-green-500'
        case 'error':
            return 'text-red-500'
        case 'warning':
            return 'text-yellow-500'
        case 'info':
            return 'text-blue-500'
        default:
            return 'text-gray-500'
    }
}
</script>

<style scoped>
.notification-enter-active,
.notification-leave-active {
  transition: all 0.3s ease;
}

.notification-enter-from {
  opacity: 0;
  transform: translateX(30px);
}

.notification-leave-to {
  opacity: 0;
  transform: translateX(30px);
}
</style>
